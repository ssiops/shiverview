...
function Manager (server, package) {
  this.server = server;
...
Manager.prototype.load = function (name, callback) {
  ...
  if (self.appNames.indexOf(name) >= 0)
    return callback({
      err: 'APP_LOADED',
      msg: 'This application is already loaded.'
    });
  else
    ...
    function (callback) {
      async.each(app.routes, function (r, callback) {
        ...
        if (self.acceptedMethods.indexOf(method) < 0)
          return callback({
            err: 'INV_METHOD',
            msg: 'Invalid HTTP method in routes.'
          });
...
Manager.prototype.unload = function (name, callback) {
  ...
  if (i >= self.apps.length)
    return callback({
      err: 'APP_NOT_LOADED',
      msg: 'The specified application is not loaded.'
    });
  var app = self.apps[i]
  async.parallel([
    function (callback) {
...
Manager.prototype.init = function (callback) {
  ...
  var patt = /^shiverview-/;
  var apps = [];
  for (var dep in self.package.dependencies) {
    if (patt.test(dep))
      apps.push(dep);
  }
